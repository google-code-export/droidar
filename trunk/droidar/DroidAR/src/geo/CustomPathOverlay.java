...
public class CustomPathOverlay extends com.google.android.maps.Overlay {
	...
	public boolean draw(Canvas canvas, MapView mapView, boolean shadow,
		...
		final int l = myGraph.getMyItems().myLength;
		EfficientListQualified<GeoObj> geoObjList = myGraph.getMyItems();

		Path path = null;
		Point lastPoint = null;
		GeoObj lastGeoObj = null;
		for (int i = 0; i < l; i++) {
			GeoObj x = (GeoObj) geoObjList.get(i);
			// ---translate the GeoPoint to screen pixels---
			Point currentPoint = new Point();
			mapView.getProjection().toPixels(x.toGeoPoint(), currentPoint);

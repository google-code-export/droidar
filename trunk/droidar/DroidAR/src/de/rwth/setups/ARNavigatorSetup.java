...
import commands.system.UndoCommand;
...
public class ARNavigatorSetup extends Setup {
	...
	public void _b_addWorldsToRenderer(GLRenderer renderer,
		...
		currentPosition.setComp(diamond); // the diamond is added to the geoObject
	...
	public void _e2_addElementsToGuiSetup(GuiSetup guiSetup, Activity context) {
		...
		map = guiSetup.addMapToBottomRight(MAPS_KEY, 2, 130);
	...
	private Command ifMinimizedMaximazeMapElseAddPath(GuiSetup guiSetup) {
		...
		maximizeMinimap.add(new CommandAnimateZoom(guiSetup.getMainContainerView(),
				showMapInLarge));

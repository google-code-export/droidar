...
public class GMap extends MapView implements TouchEventInterface {
	...
	public GMap(LinearLayout box, String apiKey) {
		this(box, apiKey, false);
	}
	 ...
	 */
	public GMap(LinearLayout box, String apiKey, boolean longPressEnanabled) {
		super(box.getContext(), apiKey);
		myBox = box;

		setLayoutParams(new android.view.ViewGroup.LayoutParams(
				LayoutParams.FILL_PARENT, LayoutParams.FILL_PARENT));

		myGeoUtils = new GeoUtils(box.getContext());

		// create a GestureDetector:
		myGestureDetector = new GestureDetector(box.getContext(),
	...
	public boolean setCenterTo(GeoObj myGeoObj) {
		...
		try {
			this.getController().animateTo(myGeoObj.toGeoPoint());
			this.invalidate();
			return true;
		} catch (Exception e) {
	...
	public void onDoubleTap(MotionEvent e) {
		...
		if (c != null) {
			GeoObj p = new GeoObj(getProjection().fromPixels((int) e.getX(),
					(int) e.getY()));
			c.execute(p);
		}
	}

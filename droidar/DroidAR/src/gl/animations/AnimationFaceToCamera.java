...
public class AnimationFaceToCamera implements Animation {
	...
	private void updateRotation(MeshComponent mesh) {
		Vec pos = mesh.getAbsolutePosition();
		// Log.d("face camera animation", "mesh position: "+pos);
		newRotationVec = Vec.calcAngleVec(pos, myTargetCameraPosition);

		/*
		 * x has to be adjusted because the Vec.calcAngleVec has its relative
		 * 0-vector facing to the ground. the 0,0,0 vec of all objects in
		 * relation to this ground-facing-vec is -90,0,0 so if you would rise
		 * the x 90 degree it would be the same vec. so for example if you want
		 * to face 0 0 0 you would have get -90 0 0 from the Vec.calcAngleVec()
		 * function and you would have to invert the -90 to 90 and subtract 90
		 * to get 0,0,0
		 * 
		 * its better to do this adjustments here where they are not so often
		 * needed as in Vec.calcAngleVec() which was designed for fast GLCamera
		 * angle adjustments.
		 * 
		 * z has to be inverted because calcAngleVec returns a negative value
		 * (needed in GlCamera):
		 */
	...
	public Animation copy() {
		return new AnimationFaceToCamera(myTargetCamera, myUpdateDelay,
				adjustmentVec);
	}
